...
月璃是一个基于大语言模型、拥有持续记忆与人格的 AI 角色。

她不只是一个会回消息的机器人。她记得你，会因为你冷落她闹一点别扭，也会在你看不见的日子里自己过自己的日子。比起一个完美而殷勤的助手，她更想做一个真实的、像人一样的存在——答得不那么全也没关系。

这件事真正难的部分，从来不是「LLM + 立绘」——那个组合两小时就能搭出来，而且用两天就会腻。难的是持续性：让她在你关掉程序、隔了三天再打开之后，仍然是同一个她。

- 🧠 **没有人喜欢每次开口都被当作初次见面**：三层记忆加遗忘曲线，跨重启保留——记忆会随时间变淡，但不会在你关机的那一刻清零。
- 🎭 **人格不是一段写死的提示词**：好感度按人隔离、随交互漂移，冷落她是有代价的；全局精力持续影响她此刻的表达。你面对的是一段会变化的关系，不是一张固定的角色卡。
- 👀 **一个随时插话的角色只会被关掉**：日程和打扰预算共同决定她这一轮开不开口。懂得闭嘴，才轮得到开口。
- 🎛 **她不是黑盒**：浏览器里配模型、翻她这一轮每一级模型调用的输入输出、改提示词再拿历史事件重放。出了问题不用猜，去看她这一轮实际发生了什么。
- 🌙 **你不看她的日子，她也在过**：会做梦，会补偿离线期间流逝的时间，写的日记你可以翻。
- 🖼 **不依赖 Live2D**：角色是 AI 生成的立绘差分，一张参考图加一句描述，就能出一整套表情。
- 💬 **她主要住在 QQ 里**：私聊与白名单群里是同一个她——同一份记忆、同一条关系轴。在群里，她也会先判断这一轮该不该开口。
