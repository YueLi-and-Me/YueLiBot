...
2. **字段级对账与版本号补齐（Python 侧，无头启动路径）**。
   `upgrade_config_directory` 每次启动都在解析之前跑（src/main.py 的配置启动段），
   解决两个沉默问题：代码新增的配置项在用户的 TOML 里没有那一行，用户
   无从得知它可配、默认值是什么；废弃字段留在文件里看似生效，实际已经
   没有代码读它。处置口径：
   - 新增字段按 schema 默认值追加进对应的表，行扫描插入而不是解析后整份
     重写——重写会丢掉用户自己写的注释和排版；
   - 段标题解析先剥离行内注释再比对段名（引号字符串里的 `#` 不是注释），
     新增与删除两条路径共用；
   - 废弃字段就地删除，删完重新解析一遍 TOML，解析失败整份还原当作没删；
     新增字段写入前同样要做回读校验；
   - 字段增删全部成功后再把 `[inner].version` 改写为当前 `CONFIG_VERSION`。
     版本号最后写，避免中途失败留下「版本已新、字段还旧」的配置；对账是
     「文件现有字段与当前模型求差」，与来源版本无关，所以 1.4.0 直接升到
     1.6.0 与逐级迁移等价，不存在逐级迁移链；
   - 写任何字节之前先把整个 `config/` 备份到 `data/backups/config/`，
     因为它含明文密钥且不入版本库，改坏没有第二份；
   - 差异汇总成一次性的控制台信息框（含版本号变化），没变化不打印。
   - 两条刻意的保守：`api_providers` / `models` 这类列表字段当叶子跳过，
     它们是用户数据不是设置项，自动补默认值会造出不存在的厂商条目；
     嵌套段整段缺失时不逐字段补，整段是一整块新功能的配置，自动铺
     默认值会让用户以为自己配过。
...
一个边界要说清：Python 读取器只接受当前版本并直接拒绝旧版本
（toml_io.py），但启动前的 `upgrade.py` 会在字段对账成功后就地把
`[inner].version` 补到当前值；因此无头部署没有 Electron 也能跨版本启动，
不再需要手工对照模板。版本对账与整份重写都以同一份 `CONFIG_VERSION` 为
契约：Electron 路径仍按版本号决定是否整份重写，Python 路径则每次启动都
与当前模型求差并补版本号，两条路径对同一批字段的结论一致。
