...
六个字段全部必填，少一个或者 `manifest_version` 不是 1 都会被拒绝加载。`id` 是
全局唯一标识，也是配置里启用/禁用时写的那个值。
...
```toml
[plugins]
disabled = ["example.hello-yueli"]
```

列出 id 即跳过该插件，**它的入口模块根本不会被执行**——跳过发生在读完清单之后、
加载之前。所以一个代码写坏了的第三方插件可以靠这一行彻底绕开，不必删目录。

用禁用名单而不是启用名单，是为了升级安全：改成启用名单会让所有既有配置在升级后
静默失去新增的内置能力，而能力消失只在模型「不会用某个工具」时才被察觉。

写了不存在的 id 不报错——第三方插件随时可能被删，为此让主体起不来不划算。真正的
反馈在启动日志：加载成功会打印「工具插件已加载」，被禁用会打印「工具插件已被配置禁用」。
