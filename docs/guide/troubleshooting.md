...
一次即可，见[从零跑起来](getting-started.md)第三步。
...
无法按当前协议解析。见 [QQ 与群聊接入](qq-setup.md)。
...
[Electron 侧的三个不报错的坑](../development/electron-pitfalls.md)。
...
分之后的多人数据不能可靠降级。详见[数据库迁移与恢复](database.md)。
