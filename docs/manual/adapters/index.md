...
1. 配置能读通 —— 配置校验通过，适配器插件被正确加载
2. 连接能建立 —— 后端终端里出现握手成功的记录，登录号与 `self_qq` 一致
3. 私聊能收发 —— 用你本人的 QQ 私聊机器人号，**QQ 里真的收到回复**
4. 群聊能收发 —— 测试群进白名单后，用协议 @ 提及她，群里也有回复

第 3 步和第 4 步要分开做。**面板上显示回复不等于 QQ 里收到**：前者只说明模型出了结果，
后者才证明出站动作被协议端接受。
