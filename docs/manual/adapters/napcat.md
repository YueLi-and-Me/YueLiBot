...
以启动日志给出的 NapCat WebUI 地址进入管理界面，不要把它与月璃面板混用。
登录后按界面要求更新管理密码，确认页面显示的 QQ 号就是机器人号。
看到管理页面不代表 OneBot WebSocket 已启用，还要配置网络服务。

## 创建正向 WebSocket 服务端

在 NapCat WebUI 打开网络配置，点「新建」，选择「WebSocket 服务端」。
它是正向连接：NapCat 等待月璃适配器来连。
不要选择 WebSocket 客户端，也不要仅启用 HTTP 服务端。
...
也不是月璃启动时打印的后端 token。
`self_qq` 必须与协议端实际登录一致，`owner.qq` 填你用来与机器人聊天的账号。
