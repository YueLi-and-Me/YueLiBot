...
def main() -> int:
    ...
    try:
        while time.time() < deadline and (port is None or token is None):
            ...
            if m:
                port = int(m.group(1))
            token_match = re.search(r"YUELI_TOKEN=([0-9a-f]{64})", line)
            if token_match:
                token = token_match.group(1)
        if port is None or token is None:
            print("[auth] 后端未启动")
            print(''.join(startup_lines))
            return 1
        ...
        for method, path, body in ENDPOINTS:
            ...
            # /health 对 Supervisor 存活探测开放；其余受保护端点必须返回 401。
            if path != "/health":
                if row[0] != 401:
        ...
        print("[auth] 除 /health 外全部要求 token：通过")
