...
覆盖中断、报错和副作用标签回灌三类历史一致性路径。
另外校验组装出的 messages 数组形状；此前只有「非空」断言，一个只发最新
用户消息的回归可以通过全部测试。
"""
...
from src.core.agent.history import (
    close_dangling_say, fit_char_budget, normalize_history, strip_say_tags,
...
def _chat(db, provider) -> ChatService:
    async def _noop(_channel: str, _payload: Any, _stream_id: int = 1) -> None:
        return None
    return ChatService(db, provider, provider, provider, _noop, cfg=Config())


def _roles(store) -> List[str]:
    return [m.role for m in store.working_memory(DESKTOP_STREAM_ID, 40)]
...
def test_strip_say_tags_keeps_multiple_bubbles_separated() -> None:
    assert strip_say_tags('<say emotion="normal">好的</say><say>稍等</say>') == (
        '好的\n稍等'
    )
    assert strip_say_tags('  <say>只有一段</say>  ') == '只有一段'
...
def test_item_budget_keeps_consecutive_user_messages_separate() -> None:
    history = [
        {'role': 'user', 'content': '应被裁掉的旧项'},
        {'role': 'user', 'content': '[当前时间]\n现在'},
        {'role': 'user', 'content': '[人物画像]\n熟人'},
        {'role': 'user', 'content': '[1] 这轮消息'},
    ]
    budget = sum(len(item['content']) for item in history[1:])

    trimmed = fit_char_budget(history, budget=budget, preserve_items=True)

    assert trimmed == history[1:]
    assert [item['role'] for item in trimmed] == ['user', 'user', 'user']
...
async def test_interrupted_turn_keeps_what_was_already_said(db) -> None:
    ...
    chat.interrupt(DESKTOP_STREAM_ID)
    await _send_and_wait(chat, '第二句')

    roles = _roles(chat.memory)
    # 关键：不能出现连续两条 user，否则历史无法保留用户上一轮输入的边界。
    assert not any(a == b == 'user' for a, b in zip(roles, roles[1:])), roles
    assert 'assistant' in roles


async def test_error_with_no_output_keeps_the_accepted_user_message(db) -> None:
    # raise_at=0：进入循环即抛出异常，流式生成器不产生任何 chunk。
    provider = _ScriptedProvider(['永远不会被 yield 的内容'], raise_at=0)
    chat = _chat(db, provider)

    await _send_and_wait(chat, '会失败的一句')

    # 消息已在 send() 确认接收时成为历史；模型零输出不能删除它，也不能伪造回复。
    assert _roles(chat.memory) == ['user']
...
async def test_messages_array_carries_full_alternating_history(db) -> None:
    ...
    for text in ['第一句', '第二句', '第三句']:
        await _send_and_wait(chat, text)
...
async def test_tone_and_samples_stay_stable_within_one_session(db) -> None:
    ...
    state = chat._sessions[DESKTOP_STREAM_ID]
    tone = state.tone
    assert len({state.seed}) == 1
    if tone:
...
async def test_new_session_after_long_gap_rerolls_persona(db) -> None:
    ...
    await _send_and_wait(chat, '第一句')
    first_seed = chat._sessions[DESKTOP_STREAM_ID].seed

    # 把上一条消息推到会话间隔之外
    from src.core.services.chat import SESSION_GAP_MS
    db.execute('UPDATE messages SET created_at = created_at - ?', (SESSION_GAP_MS * 2,))
    db.commit()

    await _send_and_wait(chat, '很久以后的一句')

    assert chat._sessions[DESKTOP_STREAM_ID].seed != first_seed, '跨过静默间隔后应该重开一段会话'
