"""验证感知层的前台程序分类、预算决策和状态监控。

屏幕感知由调用方在明确触发时执行；本模块只验证分类、预算和监控器的当前接口，
不引入后台轮询或图像采集副作用。
"""
...
import pytest
from src.desktop.classify import ForegroundInfo, classify, describe_activity
from src.core.awareness.budget import (
    DAILY_BUDGET, after_speak, after_user_spoke, decide, decide_scene, initial_state,
    InterruptContext,
)
from src.desktop.monitor import ForegroundInfo as FI, ForegroundProcessMonitor
...
class TestClassify:
    ...
    def test_describe_activity_short_session_no_span(self):
        coding = classify(ForegroundInfo('Code.exe'))
        assert describe_activity(coding, 5) == '当前用户在用 VS Code 写代码。'

    def test_describe_activity_names_the_app(self):
        """程序名必须具体到可识别的应用，无法识别画面时才能提供稳定的活动线索。"""
        assert classify(ForegroundInfo('pycharm64.exe')).app == 'PyCharm'
        assert describe_activity(classify(ForegroundInfo('pycharm64.exe')), 5) == '当前用户在用 PyCharm 写代码。'
    ...
    def test_idle_has_no_app_name(self):
        idle = classify(None)
        assert idle.app == ''
        assert describe_activity(idle, 5) == '当前用户现在没在操作电脑。'
...
class TestBudget:
    ...
    def test_last_speak_does_not_create_a_fixed_cooldown(self):
        s = after_speak(initial_state(T0), _ctx())
        assert decide(s, _ctx(now=T0 + MIN)).allow is True
    ...
    def test_high_priority_skips_budget(self):
        s = _speak_times(initial_state(T0), DAILY_BUDGET)
    ...
    def test_ignore_accumulates(self):
        s = initial_state(T0)
        for i in range(3):
            s = after_speak(s, _ctx(now=T0 + i * 5 * MIN, responded_since_last=False))
        assert s.ignored == 3
    ...
    def test_scene_reserve(self):
        from src.core.awareness.budget import SCENE_RESERVED_EVENT_SLOTS
