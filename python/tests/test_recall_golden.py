...
存在的理由：`decay.score()` 的方向曾经是反的（bm25 越相关分越低，再配合
`sort(reverse=True)`，召回的正好是候选池里匹配最差的几条），而当时的单测
`assert score(-1, 0.5) > score(-8, 0.5)` 恰好把这个错误方向钉住了。

所以这里不测公式，只测**可观察的排序结果**：给定一批事实和一个查询，
期望被排在最前面的是哪一条。公式怎么改都行，这些断言必须成立。
"""
...
from yueli.memory.store import FactInput, MemoryStore
...
def _seed(store: MemoryStore, facts: list[str]) -> None:
    for content in facts:
        store.add_fact(FactInput(content=content, kind='测试'), NOW)


def _top(store: MemoryStore, query: str, limit: int = 3) -> list[str]:
    return [f.content for f in store.recall_facts(query, limit, NOW)]
...
def test_decay_governs_the_whole_score_including_the_vector_half() -> None:
    ...
    """
    from yueli.memory.decay import relevance_from_bm25, retention_weight
