/**
 * 把 Electron 写入器的默认配置写到指定目录，供配置对齐校验（scripts/check/config_parity.py）
 * 与升级演练使用。产物就是 writeConfigDirectory 会整份重写出的那份文件。
 *
 *   npx tsx scripts/check/config_defaults.ts <目标目录>
 */
...
if (!target) {
  console.error('用法：npx tsx scripts/check/config_defaults.ts <目标目录>')
  process.exit(2)
}
