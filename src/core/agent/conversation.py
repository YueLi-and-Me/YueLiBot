...
def _call_fault_messages(reason: str) -> list[dict[str, str]]:
    ...
    两类模型侧协议错误共用本文案：网关把工具参数整段丢弃导致的空对象，以及
    枚举字段被填成自由文本。这类错误直接终局会把一次可自愈的故障变成用户可见
    的沉默。回灌只陈述拒绝原因并要求重新调用，不替模型补写参数，参数内容由模型
    重新给出。

    指引必须同时点出「填全」与「按给定取值填」两件事：

    - 现象：reason_code 被写成一整句理由时，模型重发往往换一句散文再被拒一次，
      一次纠错预算就此空耗，回合按 illegal_action 终局。
    - 原因：早期文案只说「完整填写全部必填字段」，而这类错误里字段并不缺失，
      缺的是合法取值，指引指错了方向。
    - 后果：可用取值由校验层写进 ``reason``，本函数不重复拼装；两处各写一份
      枚举必然分叉，届时回灌给模型的取值会与实际校验口径不一致。

    :param reason: 校验层给出的拒绝原因，含该动作的可用取值。
    :return: 追加到消息序列尾部的一条 user 消息，与观察回灌同一扁平格式。
    """
    return [{
        'role': 'user',
        'content': (
            '[被拒绝的工具调用]\n'
            f'原因：{reason}\n\n'
            '这次调用没有被执行。请重新发起工具调用：必填字段要填全，'
            '枚举字段只能填上面给出的取值原词，不要自造，也不要写成一句理由。'
            '只输出工具调用，不要输出解释文字。'
        ),
    }]
