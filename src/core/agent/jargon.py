...
- 打分依赖 :mod:`src.core.memory.high_frequency` 维护的会话高频词表：命中
  ...
  词条自身的出现次数浮上来。**单个 ASCII 字母或数字是唯一的例外**，它们
  连「靠自身次数浮上来」这条路都不该有，直接不参与匹配（见
  :func:`_is_noise_term`）。
...
def _is_noise_term(key: str) -> bool:
    """判断词条是否是不可能成为黑话的单个 ASCII 字符。

    单字 CJK 词条要保留：咕、典、绷、孝都是真实的群黑话形态，它们的误报由
    打分沉底处理。但**单个 ASCII 字母或数字不是黑话**，迁移数据里的
    ``a`` ``c`` ``d`` ``m`` ``x`` ``0`` ``5`` 之类是抽取产物，而不是谁真的
    在用它们表达什么。

    - 现象：这类词条在大小写不敏感的纯子串匹配下几乎命中每一条带英文或
      数字的消息，实测占全部命中的相当比例。
    - 原因：沉底机制依赖会话高频词表，而高频词表收词要求长度 ≥2，单字
      永远拿不到那份加分——对 CJK 单字这正是期望行为，因为它们至少还能靠
      自身出现次数浮上来；单个 ASCII 字符连这条路也没有，只会持续挤占
      候选位。
    - 后果：不在这里挡掉，就只能靠人工逐条降级词表，而新数据还会再产生。
