...
_ACTION_DESCRIPTIONS: Dict[str, str] = {
    'reply': '回应某条消息。只在确实要发言时使用。',
    'silent': '本轮不回应。其他人正在聊自己的事、没有可补充的内容、话题已经结束时使用。',
    'wait': '对方的话还没说完，先不表态，等对方说完再决定。',
    'react': '某条消息值得回应、但你不想开口时使用：给它贴一个表情回应即可。',
    'poke': '有人戳了你、你想回应但不想开口时使用：戳回去即可，不必每次都发消息。',
    'speak': '没有人在与你对话，但你自己有想说的内容。没有需要表达的内容时不要使用。',
    'recall': '先查询长期记忆中与某件事相关的内容，再决定本轮做什么。',
    'inspect': '先查询某个人的画像与最近互动，再决定本轮做什么。',
    'consult': '先查询你知道的知识与资料（概念、定义、事实），再决定本轮做什么。',
}
...
_WAIT_DESCRIPTIONS: Dict[str, str] = {
    'group': (
        '一句话常被拆成几条短消息连续发送。刚到的消息很短、语义不完整或明显'
        '还有后续时，使用它等后续消息合并进来后统一回应，不要逐条回复。'
        '等不到后续就视为不需要回应。'
    ),
    'direct': (
        '对方常把一句话拆成几条短消息连续发送。刚到的消息很短、语义不完整或明显'
        '还有后续时，使用它等后续消息合并进来后统一回应，不要逐条回复。'
        '等不到后续时，这批消息稍后会再次进入回合，届时必须表态，不会被搁置。'
    ),
}
...
def _tool_for(action: str, frame: DecisionFrame) -> Dict[str, Any]:
    ...
    if action in ('reply', 'speak'):
        properties['length'] = {
            ...
            'enum': ['brief', 'long'],
            'description': (
                'brief 是简短的口语回复，绝大多数时候都应选择它；'
                'long 只在对方提出需要展开的问题时选择。'
            ),
        }
        properties['reference'] = {
            'type': 'string',
            'description': (
                '提供给下一环节写正文使用的背景：为什么发言、回应哪条消息的哪句话、'
                '有哪些前因。写事实和方向，不要写台词。'
            ),
        }
