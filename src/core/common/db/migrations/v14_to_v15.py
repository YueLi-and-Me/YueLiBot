...
def _migration_needed(db: sqlite3.Connection) -> bool:
    ...
    if actual == _EXPECTED_V14:
        return True
    # 当前 DDL 可能已幂等补齐两列，但旧版本号尚未推进；只有列序、约束和默认值
    # 全部等于 v15 权威结构时，才允许跳过 ALTER 直接接管版本号。
    if actual == _EXPECTED_V15:
        return False
    raise RuntimeError(f'v14 数据库的表达方式表结构不符合预期：expressions={actual}')
...
def migrate(db: sqlite3.Connection) -> None:
    ...
    if _table_shape(db) != _EXPECTED_V15:
        raise RuntimeError('v15 迁移自检失败：expressions 结构未到达权威形态')
    ...
    if after != before:
        raise RuntimeError('v15 迁移自检失败：expressions 原有数据发生变化')

    drift = db.execute(
        'SELECT COUNT(*) FROM expressions WHERE checked != 0 OR last_used_at IS NOT NULL'
    ).fetchone()[0]
    if drift:
        raise RuntimeError('v15 迁移自检失败：新列出现非默认值')
