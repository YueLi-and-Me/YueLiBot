...
def emit_console_trace(entry: MutableMapping[str, Any]) -> None:
    ...
    一行可呈现的事件输出一行，需要并排阅读多行的事件才使用信息框。
    判据直接取 :func:`_pack_trace_rows` 的产出行数，不额外引入阈值常量：该函数
    已按控制台宽度决定字段排布。此前的规则是「无 turnId 一律出框」，真机实测
    30 小时产生 589 个框/小时，其中大多数框内仅一两个字段（如「兴趣度：11.289」），
    占用控制台且信息量低。
