...
class InnerConfig(BaseModel):
    ...
    # 因此没有任何内容变化需要迁移。反过来，bump 了才是问题：升级器对「整段缺失」
...
class DesktopPetConfig(BaseModel):
    ...
    **实验性功能，默认关闭，开启后可能遇到未知问题。** 它串联 Electron 外壳、
    前台感知与桌面主动搭话三层，是当前最容易出问题的一块。
    ...
    # 是否启用桌宠外壳。默认关闭：它是实验性功能，而且纯后端部署不该为不存在的
    # 窗口跑感知。
    enabled: bool = False
