...
黑话召回的打分用它提供量级领先的加分：命中的词条排名上升，未命中的误报沉底。
因此这张表必须反映该会话真实的高频词，而不是通用词表或机器文本的词频：

- 平台占位段（``[表情包：…]``、``[图片：…]``、``[回复 …]`` 等）是管线生成的，
  统计前剥离；
- 单条粘贴文本（整篇文章、整段代码）会使单个词重复上百次，逐条出现次数
  封顶，词频须反映多人使用而非单条长文。
...
# 消息里重复出现（「表情包」「主题」由此占据前排），不剥离的话整张表都是管线词频。
...
def strip_machine_spans(text: str) -> str:
    ...
    人打的字。高频统计与黑话匹配共用这一清洗：机器文本在任何一侧都会使统计
    失真（统计侧「表情包」占据前排，匹配侧「片」被「图片」抬高计数）。
...
def collect_terms(
    ...
    *,
    ...
    收词规则（黑话打分方案的基础，固定于此处）：CJK 词必须 ≥2 字，
    拉丁与其他 ≥2 字符。该下限不放宽：单字进不了高频词表，就得不到召回
    打分中 1000 分量级的加分，「大」「开」这类单字误报只剩自身的个位数
    计数，会被条数上限过滤；同时真单字黑话（咕、典、绷、孝）不被整批
    降级，若它在该会话真实高频，会以词条自身计数参与排序。放宽到单字入库，
    「了」「的」「大」会立即占据前排，打分沉底机制整体失效。
...
def rebuild_stream_terms(
    ...
    *,
    ...
    只统计 ``role = 'user'`` 的消息：这张表表达其他人的用法，bot 自己的
    发言不参与理解他人说法的打分。整表按会话全量重写（旧词不再
    ...
    :param window_ms: 统计窗口长度，默认 7 天：过短会使特定时段的话题独占榜单，
        过长则季节性梗难以退出。
    :param limit: 落表条数上限。
    :return: 本次落表的词条数。
