...
def now() -> int:
    ...
    :return: 自 1970-01-01 UTC 起经过的整数毫秒数。
...
def from_ms(ms: int) -> datetime:
    ...
    :return: 与时间戳对应的 UTC 感知 ``datetime``。

    :raises OverflowError: 时间戳超出平台 ``datetime`` 支持范围。
    :raises TypeError: 时间戳不是可进行除法运算的数值。
    """
...
def snapshot_date(ms: int | None = None) -> str:
    ...
    :param ms: 可选 Unix 毫秒时间戳；省略或传入 ``None`` 时读取当前系统时间。

    :return: ``YYYY-MM-DD`` 格式的本地日期字符串。
