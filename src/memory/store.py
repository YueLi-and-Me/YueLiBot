...
class MemoryStore:
    def __init__(self, db: sqlite3.Connection) -> None:
        ...
        db.execute(
            ...
            ('first_seen_at', str(current_time()))
        )
        db.commit()
    ...
    def first_seen_at(self) -> int:
        ...
        return int(row[0]) if row else current_time()

    # ------------------------------------------------------------------ L1 工作记忆
    def append_message(self, role: str, content: str, now: int | None = None) -> int:
    ...
    def delete_message(self, id: int) -> None:
        self._db.execute('DELETE FROM messages WHERE id = ?', (id,))
        self._db.commit()

    def working_memory(self, limit: int = 40) -> list[StoredMessage]:
        rows = self._db.execute(
            '''SELECT role, content, created_at FROM messages
               WHERE episode_id IS NULL ORDER BY id DESC LIMIT ?''',
            (limit,)
        ).fetchall()
        return [StoredMessage(role=r[0], content=r[1], created_at=r[2])
                for r in reversed(rows)]

    def last_message_at(self) -> int | None:
        row = self._db.execute('SELECT MAX(created_at) FROM messages').fetchone()
        return row[0] if row and row[0] is not None else None

    def interaction_density(self, now: int | None = None) -> str:
        now = now if now is not None else current_time()
    ...
    def pending_count(self) -> int:
        row = self._db.execute(
            'SELECT COUNT(*) FROM messages WHERE episode_id IS NULL'
        ).fetchone()
        return row[0] if row else 0

    def oldest_pending(self, n: int) -> list[dict[str, Any]]:
        rows = self._db.execute(
            '''SELECT id, role, content, created_at FROM messages
               WHERE episode_id IS NULL ORDER BY id ASC LIMIT ?''',
            (n,)
        ).fetchall()
    ...
    # ------------------------------------------------------------------ L2 情节记忆
    def add_episode(self, input: EpisodeInput, now: int | None = None) -> int:
        now = now if now is not None else current_time()
        cur = self._db.execute(
            '''INSERT INTO episodes (kind, summary, started_at, ended_at, created_at)
               VALUES (?, ?, ?, ?, ?)''',
            (input.kind, input.summary, input.started_at, input.ended_at, now)
        )
        ...
        if input.message_ids:
            ...
            self._db.execute(
                ...
                (episode_id, *input.message_ids)
            )
        self._db.commit()
        return episode_id

    def recent_episodes(self, limit: int = 4) -> list[RecalledEpisode]:
        rows = self._db.execute(
            'SELECT id, summary, kind, ended_at FROM episodes ORDER BY ended_at DESC LIMIT ?',
            (limit,)
        ).fetchall()
    ...
    def all_episodes(self, limit: int = 200) -> list[dict[str, Any]]:
        rows = self._db.execute(
            'SELECT id, kind, summary, ended_at FROM episodes ORDER BY ended_at DESC LIMIT ?',
            (limit,)
        ).fetchall()
        if not rows:
            return []
    ...
    def recall_episodes(self, query: str, limit: int = 3) -> list[RecalledEpisode]:
    ...
    # ------------------------------------------------------------------ L3 语义记忆
    def add_fact(self, input: FactInput, now: int | None = None) -> int:
        now = now if now is not None else current_time()
        ...
        existing = self._find_similar(content, key)
        if existing:
            ...
            self._db.execute(
                ...
                (next_strength, now, freeze_due_at(next_strength, now, existing['half_life_hours']), existing['id'])
            )
            self._db.commit()
            return existing['id']
        ...
        cur = self._db.execute(
            '''INSERT INTO facts (kind, content, content_key, strength, half_life_hours,
                                  updated_at, created_at, due_at, active)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)''',
            (input.kind, content, key, 1.0, half_life, now, now, due)
        )
    ...
    def _find_similar(self, content: str, key: str) -> dict[str, Any] | None:
        row = self._db.execute(
            '''SELECT id, content, strength, updated_at, half_life_hours
               FROM facts WHERE content_key = ?''', (key,)
        ).fetchone()
        if row:
        ...
        candidates = self._db.execute(
            '''SELECT f.id, f.content, f.strength, f.updated_at, f.half_life_hours
               ...
               WHERE facts_fts MATCH ? ORDER BY bm25(facts_fts) ASC LIMIT 8''',
            (match,)
        ).fetchall()
        for c in candidates:
    ...
    def recall_facts(self, query: str, limit: int = 6, now: int | None = None,
        ...
        rows = self._db.execute(
            '''SELECT f.id, f.kind, f.content, f.strength, f.updated_at,
                      ...
                      f.embedding
               FROM facts_fts JOIN facts f ON f.id = facts_fts.rowid
               WHERE facts_fts MATCH ? ORDER BY bm ASC LIMIT ?''',
            (match, limit * 3)
        ).fetchall()
        scored = []
        for r in rows:
        ...
        for h in result:
            ...
            if row:
                ...
                self._db.execute(
                    '''UPDATE facts SET strength = ?, updated_at = ?, due_at = ?, active = 1,
                                        hit_count = hit_count + 1, last_hit_at = ? WHERE id = ?''',
                    (nxt, now, freeze_due_at(nxt, now, row[5]), now, h.id)
                )
        if result:
            self._db.commit()
        return result
    ...
    def top_facts(self, limit: int = 8, now: int | None = None) -> list[RecalledFact]:
        now = now if now is not None else current_time()
        rows = self._db.execute(
            'SELECT id, kind, content, strength, updated_at, half_life_hours FROM facts WHERE active = 1'
        ).fetchall()
        result = []
        for r in rows:
            ret = retention(r[3], r[4], r[5], now)
    ...
    def all_facts(self, now: int | None = None) -> list[StoredFact]:
        now = now if now is not None else current_time()
        rows = self._db.execute(
            'SELECT id, kind, content, strength, updated_at, half_life_hours, due_at, active FROM facts'
        ).fetchall()
        result = []
        for r in rows:
            ret = retention(r[3], r[4], r[5], now)
    ...
    def fact_count(self) -> dict[str, int]:
        row = self._db.execute('SELECT COUNT(*), SUM(active) FROM facts').fetchone()
    ...
    def user_spoke_after(self, at: int) -> bool:
        row = self._db.execute(
            "SELECT 1 FROM messages WHERE role = 'user' AND created_at > ? LIMIT 1", (at,)
        ).fetchone()
        return row is not None
