...
class BackendClient:
    ...
    async def submit_inbound(self, event: QqInboundEvent) -> Dict[str, Any]:
        ...
        副作用：
            ...
            入站只透传引用是有意的选择：媒体的读取与描述留给主体后台完成，
            适配器不为单张图片阻塞串行入站循环。出站方向相反，图片段在
            ``segments`` 内读成字节后再交给协议端，两个方向并不对称。

            - 现象：``runner`` 把 QQ CDN 图片解析成协议端本地路径后，主体读不到
              该路径时静默降级为不理解这张图，正文停在 ``[图片]`` 占位符。
            - 原因：透传引用隐含一个前提——引用两端要能按同一个绝对路径读到同
              一个文件；协议端容器化而主体在宿主机时该前提断裂。
            - 后果：唯一留痕是主体侧 ``chat_image_source_failed``；排查入站图片
              问题须看该事件，适配器侧不会有任何日志。
        """
        client = self._http
        if client is None:
